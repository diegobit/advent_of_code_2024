...
func (s Set) Add(el Move) {
	s[el] = struct{}{}
}

func (s Set) Contains(el Move) bool {
	_, ok := s[el]
	return ok
}
...
func dijkstra(maze *Maze) (dists map[State]int, prevs map[State]*State, finalState State) {
	...
	otherCandidates := []State{
		State{maze.end.y, maze.end.x, Dir{0, -1}},
		State{maze.end.y, maze.end.x, Dir{-1, 0}},
		State{maze.end.y, maze.end.x, Dir{+1, 0}},
	}
...
func drawPath(maze *Maze, prevs map[State]*State, finalState State) (mazeTraced *Maze) {
	mazeTraced = maze.Clone()
	curr := prevs[finalState]
	for curr != nil {
		if (*mazeTraced.mat)[curr.y][curr.x] == "." {
			(*mazeTraced.mat)[curr.y][curr.x] = dir2str(curr.dir)
		}
		curr = prevs[*curr]
	}
	return
...
func simulate(maze *Maze) (minPathCost int) {
	dists, prevs, finalState := dijkstra(maze)

	mazeTraced := drawPath(maze, prevs, finalState)
